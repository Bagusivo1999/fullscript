...
"""
╔══════════════════════════════════════════════════════════╗
║  ⚡ VOLTLY EARN BOT — STOP DI MONETAG LIMIT            ║
║  🔐 Login via InitData                                 ║
║  📺 AdsGram (skip 400) • Monetag (patokan stop)       ║
║  🛑 Stop jika Monetag 10/10                           ║
║  👑 Owner: @MoneyMaker_w                              ║
╚══════════════════════════════════════════════════════════╝
"""
...
║   {GOLD}  ╚═══╝   ╚═════╝ ╚══════╝╚═╝      ╚═╝   {PURPLE}        ║
║                                                          ║
║   {CYAN}⚡ VOLTLY EARN ⚡{PURPLE}     ║
║   {LIME}📺  Auto Ads  {PURPLE}║
║   {LIME}🛑 Bot akan Stop jika Limit                    {PURPLE}║
║                                                          ║
║   {PINK}👑 Owner : @MoneyMaker_w                          {PURPLE}║
║   {DIM}📦 Version : 2.0                  {PURPLE}║
║                                                          ║
╚══════════════════════════════════════════════════════════╝{RESET}
""")
...
class VoltlyBot:
    ...
    def get_balance(self):
        ...
        if resp.status_code == 200:
            try:
                data = resp.json()
                user = data.get('user', {})
                self.balance_usdt = user.get('balanceUsdt', 0)
                self.total_earned_usdt = user.get('totalEarnedUsdt', 0)
                print(f"{GREEN}└─ ✅ Balance: {self.balance_usdt} USDT | Total Earned: {self.total_earned_usdt} USDT{RESET}")
                return data
            except:
    ...
    def claim_ad(self, token, provider):
        ...
        if resp.status_code == 200:
            try:
                ...
                if reward:
                    print(f"{GREEN}└─ ✅ Claimed {provider} +{reward} V (VOLT){RESET}")
                else:
    ...
    def watch_adsgram(self):
        ...
        if not token:
            print(f"{RED}❌ Gagal start AdsGram{RESET}")
            return False
    ...
    def watch_monetag(self):
        ...
        if not token:
            print(f"{RED}❌ Gagal start Monetag{RESET}")
            return False
    ...
    def is_monetag_done(self):
        return self.mt_count >= MAX_MONETAG
...
def start_farming(config):
    ...
    print(f"{CYAN}🚀 Starting farming...{RESET}")
    print(f"{YELLOW}📺 AdsGram: {bot.ag_count}/{MAX_ADSGRAM} | Monetag: {bot.mt_count}/{MAX_MONETAG}{RESET}")
    print(f"{YELLOW}⏱️ Timer antar iklan: {TIMER_ADS}s, antar cycle: {TIMER_CYCLE}s{RESET}")
    print(f"{YELLOW}🛑 Bot akan berhenti jika Monetag mencapai {MAX_MONETAG}/10{RESET}")
    ...
    try:
        while True:
            ...
            # Cek Monetag limit (patokan stop)
            if bot.is_monetag_done():
                print(f"\n{GREEN}✅ Monetag sudah mencapai limit {MAX_MONETAG}/10!{RESET}")
                print(f"📊 AdsGram: {bot.ag_count}/{MAX_ADSGRAM} | Monetag: {bot.mt_count}/{MAX_MONETAG}")
                bot.get_balance()
                print(f"{YELLOW}🛑 Bot berhenti. Kembali ke menu...{RESET}")
                break

            # AdsGram
            print(f"\n{YELLOW}📺 AdsGram ({bot.ag_count}/{MAX_ADSGRAM})...{RESET}")
            bot.watch_adsgram()
            bot.countdown(TIMER_ADS, "⏳ Jeda sebelum iklan")

            # Monetag
            print(f"\n{YELLOW}📺 Monetag ({bot.mt_count}/{MAX_MONETAG})...{RESET}")
            bot.watch_monetag()
            bot.countdown(TIMER_ADS, "⏳ Jeda sebelum iklan")
